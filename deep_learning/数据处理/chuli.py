...
def show_label(image_path,png_path):
    ...
    image = Image.blend(png,image,0.3)
    # image.show()
...
def show_png_xml(soource_dir):
    ...
    for i in os.listdir(soource_dir):
        image_files = os.path.join(soource_dir,i,'img')
...
def show_png_xml_singal(source_dir):
    image_path = os.path.join(source_dir)
    png_path =os.path.join(image_path.split('.')[0]+'.png')
    xml_path=os.path.join(image_path.split('.')[0]+'.xml')
    image = show_label(image_path=image_path, png_path=png_path)
    save_path = os.path.join(image_path.split('.')[0]+'1.jpg')
    show_xml(image=image, xml_path=xml_path, save_path=save_path)
    # img = Image.open(image_files)
    pass

if __name__ == '__main__':
    source_path ='G:DATA/MNC_data_style/process_new2'
    save_path='G:DATA/MNC_data_style/process_new2'
    show_png_xml(source_path)
    # show_png_xml_singal('C:/Users/Administrator/Desktop/000455.jpg')